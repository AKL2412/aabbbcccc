package com.gp.dao;

import org.hibernate.Criteria;
import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.hibernate.Transaction;
import org.hibernate.criterion.Restrictions;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

import com.gp.domain.Avance;
@Repository("avanceDao")
public class AvanceDaoImpl implements AvanceDao {

	@Autowired
	private SessionFactory sessionfactory;
	@Override
	public Avance trouverParId(Integer num) {
		Criteria crit = sessionfactory.getCurrentSession().createCriteria(Avance.class);
		crit.add(Restrictions.eq("avanceId", num));
		return (Avance) crit.uniqueResult();
	}

	@Override
	public void enregistrer(Avance avance) {
		Session sess=sessionfactory.openSession();
		Transaction tr = sess.beginTransaction();
		sess.saveOrUpdate(avance);
		tr.commit();
		sess.close();

	}

	@Override
	public boolean supprimer(Avance avance) {
		try{
			Session sess=sessionfactory.openSession();
			Transaction tr = sess.beginTransaction();
			sess.delete(avance);
			tr.commit();
			sess.close();
			return true;
		}catch(Exception e){
			System.out.println(e.getMessage());
			return false;
		}
	}

}
